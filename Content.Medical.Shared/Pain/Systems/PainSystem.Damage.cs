...
public partial class PainSystem
{
    ...
    /// <summary>
    /// Helper to update or add a pain modifier.
    /// </summary>
    public bool ChangePainModifier(
        Entity<NerveSystemComponent?> brain,
        EntityUid nerve,
        string identifier,
        FixedPoint2 change,
        TimeSpan? time = null,
        PainDamageTypes painType = PainDamageTypes.WoundPain)
    => TryChangePainModifier(brain, nerve, identifier, change, brain.Comp, time, painType) ||
        TryAddPainModifier(brain, nerve, identifier, change, painType, brain.Comp, time);
    ...
    {
        if (!Resolve(nerveUid, ref nerveComp, false) || _body.GetBody(nerveUid) is not {} body)
            return;
